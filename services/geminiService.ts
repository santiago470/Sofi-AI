...
// Generates the system instruction based on user info and chat mode
const getSystemInstruction = (userInfo: UserInfo, mode: ChatMode): string => {
  ...
  // For artist mode, system instruction is handled via prompt engineering in the generation call
...
// Generates the model configuration based on the chat mode
...
// Main function to interact with the Gemini API for text
export const generateSofiResponse = async (
  ...
  if (!process.env.API_KEY) {
      ...
      throw new Error('Chave de API não encontrada. Por favor, configure-a para continuar.');
    }
  ...
  const formattedHistory = (history || [])
    .filter(msg => !msg.structuredData) // Don't include structured data in history
    .map((msg) => ({
...
export const generateSofiImage = async (prompt: string): Promise<string> => {
    ...
    // Enhance prompt for a consistent, cute style, and to be more resilient to safety filters.
    ...
    const response = await ai.models.generateImages({
        ...
        config: {
          ...
          aspectRatio: '1:1', // Square images are cute
        },
    });
    ...
    if (response.generatedImages && response.generatedImages.length > 0 && response.generatedImages[0].image.imageBytes) {
        ...
        // Log the response for better debugging in case of failure (e.g., safety blocks)
        console.error("Image generation failed. Full response object:", JSON.stringify(response, null, 2));
        
        // Check if generation was blocked for safety. The `GeneratedImage` object might have a `finishReason`.
        // If the finishReason is 'SAFETY', we can assume it was blocked for safety reasons.
