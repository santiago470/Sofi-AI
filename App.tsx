...
const App: React.FC = () => {
  ...
  const handleSendMessage = async (text: string, fromVoice: boolean = false) => {
    ...
    try {
      ...
      const errorMessageText = chatMode !== ChatMode.Psychologist
        ? 'Awn, o meu cérebro de IA deu um tilt! 🥺 Não consegui processar isto. Podemos tentar de novo?'
        : 'Peço desculpa, ocorreu um erro ao processar o teu pedido. Vamos tentar abordar isto de outra forma.';
      ...
      setMessages(prev => [...prev, errorMessage]);
       ...
       setError('Oh não! 🥺 Tive um probleminha para falar com a IA. Verifica a tua ligação ou se a API está configurada corretamente no servidor e tenta de novo!');
    } finally {
      setIsLoading(false);
    }
  };
